...
public class TitleManager {
     ...
     * Initializes the GamePanel reference, loads title images, and sets up the initial map.
     *
	 ...
	 * Assumes a specific format in the file, where numbers represent different map titles.
	...
	public void Draw(Graphics2D g2)
	{
		...
		// Draw individual map tiles based on their positions in the 'mapTitleNum' matrix.
		for (int i = 0; i < mapTitleNum.length; i++) 
		{
			for (int j = 0; j < mapTitleNum[i].length; j++) 
			{
				...
				if (mapTitleNum[i][j].mapTitle == 1)
					g2.drawRect(mapTitleNum[i][j].coll.rec.x, mapTitleNum[i][j].coll.rec.y, mapTitleNum[i][j].coll.rec.width, mapTitleNum[i][j].coll.rec.height);
			}
		}
	}
	...
	public Title GetTitleFromPos(Rectangle rec)
	{
		for (int i = 0; i < mapTitleNum.length; i++) 
		{
			for (int j = 0; j < mapTitleNum[i].length; j++) 
